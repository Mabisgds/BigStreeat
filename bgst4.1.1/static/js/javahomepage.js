...
const engine = (function () {
    ...
    const formData = {
        nome: document.getElementById('court_nome').value,
        cidade: document.getElementById('court_cidade').value,
        bairro: document.getElementById('court_bairro').value,
        estado: document.getElementById('court_estado').value,
        rua: document.getElementById('court_rua').value,
        cep: document.getElementById('court_cep').value,
...
if (navExploreCourts) {
    ...
    try {
        ...
        const response = await fetch("http://127.0.0.1:5000/quadras", {
            method: "POST",
    ...
    container.innerHTML = "<p>Carregando quadras...</p>";
    ...
    try {
        ...
        if (!quadras.length) {
            container.innerHTML = "<p>Nenhuma quadra cadastrada.</p>";
            return;
        }
        ...
        quadras.forEach(q => {
            ...
            card.innerHTML = `
                <h3>${q.nome}</h3>
                <p><strong>Endereço:</strong> ${q.rua}</p>
                <p><strong>Bairro:</strong> ${q.bairro}</p>
                <p><strong>Cidade:</strong> ${q.cidade} - ${q.estado}</p>
        ...
        container.innerHTML = "<p>Erro ao carregar quadras.</p>";
    }
}
// --------------------
    // EXPOSIÇÃO DO MÓDULO
