...
// 👇 INÍCIO DA MUDANÇA: BOTÃO DO MAPA ADICIONADO 👇
                    <button class="btn-card-secondary-outline" 
                            onclick="engine.logic.abrirLocalizacao('${ev.rua || ""}', '', '${ev.bairro}', '${ev.cidade}', '')" 
...
// 👇 INÍCIO DA MUDANÇA: FUNÇÃO DE REDIRECIONAMENTO MAPS/PYTHON 👇
    const _abrirLocalizacao = async (rua, numero, bairro, cidade, estado) => {
        try {
            _showToast("Buscando rota...");
            const dadosEndereco = { rua, numero, bairro, cidade, estado };

            const response = await fetch("http://127.0.0.1:5000/api/gerar-mapa", {
                method: "POST",
...
// 👇 INÍCIO DA MUDANÇA: EXPOSIÇÃO DA FUNÇÃO 👇
