...
def criar_evento():
    ...
    try:
        ...
        valores = (
            ...
            dados.get("banco") or "",
            dados.get("numero_evento") or "",
            dados.get("rua_evento") or "",
        ...
        sql = """
            INSERT INTO eventos (
                ...
                beneficiario, banco, numero_evento, rua_evento, cidade_evento,
...
# Dashboard resumo: cards apenas estética no front (sem API)
def _unused_dashboard_resumo():
    db = conectar_banco()
    if db is None:
        return jsonify({"erro": "Erro conexão banco"}), 500
    ...
    try:
        ...
        # Partidas ganhas: se existir tabela/coluna no futuro, usar. Por ora retorna 0.
        partidas_ganhas = 0
        try:
            cursor.execute(
                "SELECT partidas_ganhas FROM usuario WHERE id_usuario = %s",
                (usuario_id,)
            )
            row = cursor.fetchone()
            if row and row.get("partidas_ganhas") is not None:
                partidas_ganhas = int(row["partidas_ganhas"])
        except Exception:
            pass

        # Eventos criados pelo usuário
        cursor.execute(
            "SELECT COUNT(*) AS total FROM eventos WHERE usuario_id = %s",
            (usuario_id,)
        )
        row = cursor.fetchone()
        eventos_criados = row["total"] if row else 0

        # Quadras do usuário (cadastradas por ele) – “mais frequentadas” = suas quadras
        cursor.execute(
            """SELECT nome_quadra FROM quadra
               WHERE usuario_id = %s
               ORDER BY nome_quadra
               LIMIT 5""",
            (usuario_id,)
        )
        quadras_rows = cursor.fetchall()
        quadras_mais_frequentadas = [
            {"nome": r["nome_quadra"], "total_eventos": 0}
            for r in (quadras_rows or [])
        ]
        ...
        return jsonify({
            ...
            "quadras_mais_frequentadas": quadras_mais_frequentadas
        }), 200
...
# (Dashboard resumo: apenas estética no front; sem API)
def _dashboard_resumo_removed():
    """Retorna resumo para os cards do dashboard: partidas ganhas, quadras mais frequentadas, eventos criados."""
